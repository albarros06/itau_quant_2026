...
- [ ] T016 [P] `tests/contract/test_activity_log_append_only.py` — asserts `store/repository.py` (T011) contains no `UPDATE`/`DELETE` against `activity_log`, only `INSERT`/`SELECT` (FR-014, contracts/activity-log-contract.md rule 1); also asserts `record_activity`'s call sites never receive a resolved credential value in `target`/`reason` (contracts/activity-log-contract.md rule 5)
- [ ] T017 [P] `tests/contract/test_budget_enforcement.py` — asserts `budget.py` (T013) raises `BudgetExhausted` at the configured limit, blocks further discretionary calls for the remainder of the period, and resets only at the next `period_key` (contracts/budget-contract.md); covers both the `llm` and `vendor_request` kinds so a discovery-side regression (e.g. an unguarded call site) is caught here too (depends on T013)
...
- [ ] T020 [P] [US1] `src/ops_agent/proposals/models.py` — `ProvisioningProposal` pydantic `StrictModel` per data-model.md; approval/rejection is a status transition on this model (`decided_by`/`decided_at`/`applied_commit_sha`/`status`), not a separate `ApprovalDecision` entity (data-model.md `ApprovalDecision`)
...
- [ ] T045 Run 001's full existing test suite (`tests/contract`, `tests/integration`, `tests/unit` under `energy_research`) unchanged and confirm 100% pass after 002's additive touches to `ingestion/providers/*.py`, `ingestion/registry.py`, and `config/settings.py` (SC-008) (depends on T005–T007, T033–T034)
